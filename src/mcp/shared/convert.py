...
def _populate_result_from_content_block(content_block: dict, result: mcp_pb2.CallToolResponse.Result) -> bool:
    ...
    match content_type:
        ...
        case "resource":  # EmbeddedResource
            ...
            if "resource" in embedded_resource_data and isinstance(embedded_resource_data["resource"], dict):
...
def _handle_dict_tool_output(tool_output: dict):
    ...
    if "structuredContent" in tool_output and len(tool_output) == 1:
        result = mcp_pb2.CallToolResponse.Result()
        try:
            json_format.ParseDict(tool_output["structuredContent"], result.structured_content)
        except json_format.ParseError as e:
            logger.error("Failed to parse structuredContent from dict: %s", e, exc_info=True)
        return result
    ...
    if "content" in tool_output or "structuredContent" in tool_output or "isError" in tool_output:
        ...
        if "structuredContent" in tool_output and tool_output["structuredContent"] is not None:
            result = mcp_pb2.CallToolResponse.Result()
            if is_error:
        ...
        if "content" in tool_output and isinstance(tool_output["content"], Sequence):
...
def proto_result_to_types_result(
    ...
    for proto_result in proto_results:
        if proto_result.HasField("text"):
            ...
            content.append(
                types.ResourceLink(
                    name=proto_result.resource_link.name, type="resource_link", uri=proto_result.resource_link.uri
                )
            )

        if proto_result.HasField("structured_content"):
            structured_content = json_format.MessageToDict(proto_result.structured_content)
