...
def test_tool_proto_to_type_empty_schemas():
  ...
  expected_tool_type = types.Tool(
      name="empty_tool", description="No schemas", inputSchema={}, outputSchema={}
  )
...
def test_tool_output_to_proto_tuple():
  ...
  assert converted_proto[1].structured_content == json_format.ParseDict(
...
def test_tool_output_to_proto_list_of_other_values():
  ...
  assert converted_proto[0].structured_content == json_format.ParseDict(
      {"result": 1}, struct_pb2.Struct()
  )
  assert converted_proto[1].structured_content == json_format.ParseDict(
      {"result": 2}, struct_pb2.Struct()
  )
  assert converted_proto[2].structured_content == json_format.ParseDict(
      {"result": 3}, struct_pb2.Struct()
  )
...
def test_tool_output_to_proto_float():
  ...
  assert (
      pytest.approx(
          converted_proto[0].structured_content.fields["result"].number_value
      )
      == 123.45
  )
...
def test_tool_output_to_proto_bad_structured_content():
  ...
  with unittest.mock.patch.object(
      json_format, "ParseDict", side_effect=json_format.ParseError("Bad JSON")
  ):
    converted_proto = convert.tool_output_to_proto(tool_output)
  assert len(converted_proto) == 1
...
def test_tool_output_to_proto_model_dump_with_type():
  ...
  assert len(result) == 1
  assert result[0].structured_content == json_format.ParseDict(
      {"type": "mock", "value": 123}, struct_pb2.Struct()
  )
